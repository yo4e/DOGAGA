...
|---|---|---|---|
| 形式を識別できない | このファイルの形式を確認できませんでした。別のファイルを選ぶか、MP4に変換してからもう一度お試しください。 | 別素材または変換 | sniff結果、magic bytesの分類、parser error |
| 映像codec構成が使えない | この動画の映像形式は、現在の環境では読み込めません。H.264のMP4に変換すると読み込める場合があります。 | H.264 MP4へ変換、推奨環境を確認 | exact codec string、decoder config、API結果 |
| 音声codec構成が使えない | この動画の音声形式は、現在の環境では読み込めません。AAC音声を含むMP4に変換してからお試しください。 | AAC-LCへ変換 | exact codec string、sample rate、channels、API結果 |
| 初期対象外の解像度・fps・HDR | この動画は現在の対応範囲を超えています。1080p・30fps・SDRに変換してからお試しください。 | 初期上限へ変換 | width、height、fps、bit depth、color metadata |
| 破損 | ファイルを最後まで読み込めませんでした。元のアプリから書き出し直すか、別のコピーをお試しください。 | 再取得または再書出し | 失敗offset、track、例外 |
| 資源不足 | 処理に必要なメモリを確保できませんでした。ほかのタブを閉じるか、短い・小さい動画でお試しください。 | 負荷を下げて再試行 | queue、memory、frame保持数 |
| encoderが使えない | この環境では、選んだ設定で動画を書き出せません。推奨ブラウザを確認するか、設定を下げてお試しください。 | 推奨環境または低い設定 | encoder config、API結果、例外 |
| muxまたは保存に失敗 | 動画ファイルを作成できませんでした。プロジェクトはそのままです。設定を確認して、もう一度お試しください。 | 再試行、診断詳細を確認 | encoder完了の有無、mux stage、保存例外 |
