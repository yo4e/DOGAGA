...
The repository root [README](../README.md) is also English and describes the current public feature set, setup, limitations, and WebMCP surface.
...
Those files are useful development history, but some describe planned features such as persistence, waveform editing, lyrics/captions, or earlier single-track WebMCP stages that are **not part of the current submitted build**.
...
This avoids rewriting repository history while giving reviewers a clear English path through the current product.